...
const { data: category } = useCategory(Number(id.value))
const { data: jobs } = useCategoryJobs(Number(id.value), searchParams)
const { data: favouritedJobs } = useFavouritedJobs()

const activeJobsFallback = computed(() => {
  return `${jobs.value?.length} active ${jobs.value?.length !== 1 ? 'jobs' : 'job'} in ${category.value?.name}`
})
